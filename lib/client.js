...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function StatsPill(props) {
      ...
      const rate = Number.isFinite(cfg.usdCnyRate) && cfg.usdCnyRate > 0 ? cfg.usdCnyRate : 6.82
      const costText = cost !== null
        ? (cfg.costCurrency === 'cny' ? '¥' + formatCost(cost * rate) : '$' + formatCost(cost))
        : null
      const balText = bal !== null
        ? (cfg.balanceCurrency === 'cny' ? '¥' + bal.balance : '$' + (Number(bal.balance) / rate).toFixed(2))
        : null
      const parts = []
      if (cfg.showBalance) parts.push(balText !== null ? balText : '余额 --')
      if (cfg.showCost) parts.push(costText !== null ? '本次 ' + costText : '本次 --')
      const barLabel = parts.length > 0 ? parts.join(' · ') : '会话统计'

      const rows = []
      if (win !== null) rows.push(['当前时段', win === 'peak' ? '高峰（工作日 09-12/14-18）· 峰价' : '空闲（半价，周末全天）'])
      ...
      if (costText !== null) rows.push(['花费', costText])
      rows.push(['余额', balText !== null ? balText : '--'])
    ...
    function CostBalanceSettings() {
      ...
      return React.createElement('div', { className: 'cbpForm' }, [
        ...
        React.createElement('div', { className: 'cbpForm_row', key: 'costcur' }, [
          React.createElement('span', { className: 'cbpForm_label' }, '花费货币'),
          React.createElement('select', { value: d.costCurrency, onChange: (e) => update({ costCurrency: e.target.value }) }, [
            React.createElement('option', { value: 'cny', key: 'cny' }, '人民币 ¥'),
            React.createElement('option', { value: 'usd', key: 'usd' }, '美元 $'),
          ]),
        ]),
        React.createElement('div', { className: 'cbpForm_row', key: 'balcur' }, [
          React.createElement('span', { className: 'cbpForm_label' }, '余额货币'),
          React.createElement('select', { value: d.balanceCurrency, onChange: (e) => update({ balanceCurrency: e.target.value }) }, [
            React.createElement('option', { value: 'cny', key: 'cny' }, '人民币 ¥'),
            React.createElement('option', { value: 'usd', key: 'usd' }, '美元 $'),
          ]),
        ]),
        React.createElement('div', { className: 'cbpForm_row', key: 'rate' }, [
          React.createElement('span', { className: 'cbpForm_label' }, '汇率（USD→CNY）'),
          React.createElement('input', { type: 'number', min: 1, max: 20, step: 0.01, value: d.usdCnyRate, onChange: (e) => update({ usdCnyRate: Number(e.target.value) }) }),
        ]),
